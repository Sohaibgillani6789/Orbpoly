...
Current grass shader.
Front-facing grass only.
Reduced fragment shader math.
Reduced distant wind calculation.
Reduced texture sampling.
Mobile render scale of 1.0.
...
Open the game in Chrome Android in landscape orientation.
Start with the low-end mobile pixel ratio of 1.0.
Record menu frame time for 60 seconds.
Start a bot match.
Test normal movement, jumping, combat, ring-out, and day/night transition.
Test the heaviest combat effects.
Continue for at least 15 minutes to detect thermal throttling.
Background the browser and return to the game.
Rotate the device and return to landscape.
Repeat with multiplayer if enabled.
Acceptance criteria
...
Replace or reduce IcosahedronGeometry(50, 15).
Centralize pixel ratio for the main renderer, preview renderer, and particles.
Start low-end mobile at pixel ratio 1.0.
Add frame-time measurement and adaptive render scaling.
Throttle and dispose the character preview renderer.
Profile grass vertex cost, fragment cost, and overdraw.
Optimize sky and grass shader math.
Verify Cannon-es runs once per frame and pauses correctly.
Add shared model and texture loading caches.
Reduce DOM/HUD updates and per-frame allocations.
Audit transparent effects and particle overdraw.
Measure the production build on the target device for 15 minutes.
Re-test after every performance change to prevent visual or gameplay regressions.
