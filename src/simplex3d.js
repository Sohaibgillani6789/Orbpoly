/**
 * 3D Simplex Noise — Optimized for procedural geometry generation
 * Based on Stefan Gustavson's implementation (public domain)
 * 
 * Key optimizations:
 *  - Pre-computed gradient table (no per-sample branching)
 *  - Bit-manipulation permutation hash
 *  - Inlined dot products
 */
...
// Seed with a fixed permutation for deterministic results
...
/**
 * Fractal Brownian Motion — multi-octave noise for natural-looking displacement
 * @param {number} x 
 * @param {number} y 
 * @param {number} z 
 * @param {number} octaves - number of noise layers (4-6 for rock)
 ...
 */
export function fbm3D(x, y, z, octaves = 5, lacunarity = 2.0, persistence = 0.5) {
    let value = 0;
    let amplitude = 1.0;
    let frequency = 1.0;
    let maxValue = 0;
